      ...
      if (data) {
        form.addEventListener("submit", async function (e) {
          ...
          if (
            ...
            errors.push(
              "Vous ne pouvez pas retirer plus que votre solde. Solde actuel: " +
          ...
          if (errors.length > 0) {
            ...
            try {
              ...
              if (response.ok) {
                if (
                  data.account.balance - amountByType < data.account.lowsale &&
                  typeSelect.value === "withdrawal"
                ) {
                  localStorage.setItem(
                    "warningMessage",
                    "Votre solde est inférieur au seuil défini. Veuillez recharger votre compte.",
                  );
                }
